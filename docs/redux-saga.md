...
function* decreaseSaga() {
  yield delay(1000); // 1초를 기다립니다.
  yield put(decrease()); // 특정 액션을 디스패치 합니다. DECREASE_ASYNC dispatch => 사가가 지켜보다 해당하는 동작을 함 => DECREASE dispatch 사가가 지켜보는게 없다 => reducer로 가서 DEACRESE action type에 해당하는 작업을 한다. 
}
